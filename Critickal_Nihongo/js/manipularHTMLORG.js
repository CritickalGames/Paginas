import * as etiqueta from "./etiquetasConst "

export function articulo(xml) {
...
export function mkTxtDeAtr(CrearEtiquetaHTML, BuscarTagXML, AgregarAtributo, index) {
let hijo = melement(CrearEtiquetaHTML);
hijo.appendChild(wnode(getAttribute(BuscarTagXML, AgregarAtributo, index)));
return hijo;
}

export function getAttribute(tagXML, atributo, index) {
return tagXML[index].getAttribute(atributo);
}

export function melement(elemento) {
return document.createElement(elemento);
}

export function addTxtNodesMem(elementoHTML, elementosXML, index) {
return elementoHTML.appendChild(wnode(elementosXML[index].childNodes[0].nodeValue));
}

export function wnode(texto) {
return document.createTextNode(texto);
}

export function crearTablo(parrafoElemento) {
...
export function crearDiv(parrafoDiv) {
div = melement(di);
